...
export function PerformanceCharts({ battingPerformances, bowlingPerformances }: PerformanceChartsProps) {
        ...
        <CardContent className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={battingData}>
              ...
              <XAxis dataKey="match" label={{ value: "Matches", position: "bottom" }} />
              <YAxis yAxisId="left" label={{ value: "Runs", angle: -90, position: "insideLeft" }} />
              <YAxis yAxisId="right" orientation="right" label={{ value: "Strike Rate", angle: 90, position: "insideRight" }} />
              <Tooltip />
              <Line yAxisId="left" type="monotone" dataKey="runs" stroke="hsl(var(--chart-1))" />
              <Line yAxisId="right" type="monotone" dataKey="strikeRate" stroke="hsl(var(--chart-2))" />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
        ...
        <CardContent className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={bowlingData}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
              <XAxis dataKey="match" label={{ value: "Matches", position: "bottom" }} />
              <YAxis yAxisId="left" label={{ value: "Wickets", angle: -90, position: "insideLeft" }} />
              <YAxis yAxisId="right" orientation="right" label={{ value: "Economy", angle: 90, position: "insideRight" }} />
              <Tooltip />
              <Bar yAxisId="left" dataKey="wickets" fill="hsl(var(--chart-3))" />
              <Line yAxisId="right" type="monotone" dataKey="economy" stroke="hsl(var(--chart-4))" />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
